...
Load file containing pfam info and metadata.
...
Filter out pfam, read gene calls, and combine contig info with pfam info.
```{r, warning=FALSE, message=FALSE}
filtered_pfam <- pfam %>% 
...
Add bin column and filter only manually curated MAGs.
```{r, warning=FALSE, message=FALSE}
# extract bin info
...
Load df with list of MAGs whose mean rel abundance shifted significantly between treatments.
```{r, warning=FALSE, message=FALSE}
...
Next, calculate relative proportion of pfams.
```{r, warning=FALSE, message=FALSE}
...
Split into resistant and suceptible categories
```{r, warning=FALSE, message=FALSE} 
...
Perform Wilcoxon tests on relative proportions for Pfams in both groups
```{r, warning=FALSE, message=FALSE}
...
# save df with mean proportions of enriched Pfams
#write.csv(mean_proportions_significant, "../R output/enriched_pfams.csv")
```


```{r, warning=FALSE, message=FALSE}
# replace labels for the all shared pfams (4226 unique pfams) and save 
...
Figure 2B: Plot highlighting select examples that show existence of multiple mechanisms of predation resistance 
```{r, warning=FALSE, message=FALSE}
...
Plot Figure 2C: PCoA of 23 enriched pfams for all populations to see if any clustering 
```{r, warning=FALSE, message=FALSE}
...
# calculate variation explained by the first two axes
...
Plot 2C
```{r, warning=FALSE, message=FALSE}
# combine PCoA coordinates with bin and category info
...
# Calculate percentages of variation explained by the first two axes for the labels
...
Plot S2A: Differential abundance of 23 overrepresented Pfams
```{r, warning=FALSE, message=FALSE}
custom_colors <- c("Resistant" = "#B22222", "Susceptible" = "#808080")
...
Plot S2B: Plot of top 10 Eigen values from the overrepresented Pfam PCoA
```{r, warning=FALSE, message=FALSE}
...
Plot S2C: Plot PCoA of 23 enriched pfams only for only resistant populations to see if there is clustering based on taxonomy
```{r, warning=FALSE, message=FALSE}
# prepare data
...
Permanova on Pfams of resistant mags based on taxonomy
```{r, warning=FALSE, message=FALSE}
...
```
Permanova indicates significant difference in profiles based on taxonomy. 
But, since there is uneven number of taxa within phyla, check if taxonomic groups differ in dispersion to validate PERMANOVA
```{r}
dispersion_test <- betadisper(bray_curtis_dist, phylum_info_filtered$t_phylum)
anova(dispersion_test)
```
Here, p < 0.05 suggests that dispersion differences may affect clustering.
